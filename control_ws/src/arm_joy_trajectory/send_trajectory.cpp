...
int main(){
  ...
  auto pub = node->create_publisher<trajectory_msgs::msg::JointTrajectory>(
    "cmd/arm/joint_trajectory", 10);
  
  auto twist_callback =
    [this](const std::shared_ptr<geometry_msgs::msg::Twist> twist_msg) -> void {
      float linear_commands_x = twist_msg.linear.x;
      float linear_commands_y = twist_msg.linear.y;
      float linear_commands_z = twist_msg.linear.z;
      float angular_commands_z = twist_msg.angular.z;
      float angular_commands_y = twist_msg.angular.y;
      float angular_commands_x = twist_msg.angular.x;
      new_msg_ = true;
  };

  joint_twist_subscriber_ =
    get_node()->create_subscription<geometry_msgs::msg::Twist>(
      "cmd/arm", rclcpp::SystemDefaultsQoS(), twist_callback);
  ...
  robot_tree.getChain("body_link", "arm_gripper_link", chain);
