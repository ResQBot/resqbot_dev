...
//+
    fl_cmd_ = fl_cmd_ + joint_velocities_command_[0] * period.seconds() * 360/12;
//+
    fr_cmd_ = fr_cmd_ + joint_velocities_command_[1] * period.seconds() * 360/12;
//+
    rl_cmd_ = rl_cmd_ + joint_velocities_command_[2] * period.seconds() * 360/12;
//+
    rr_cmd_ = rr_cmd_ + joint_velocities_command_[3] * period.seconds() * 360/12;
    
    
//+
    std::string flipper_answer_ =
//+
      "FL" + std::to_string(rr_cmd_) +
//+
      "FR" + std::to_string(fr_cmd_) +
//+
      "RL" + std::to_string(rl_cmd_) +
//+
      "RR" + std::to_string(fl_cmd_);

    //std::cout << flipper_answer_ << "\n";

    sscanf(flipper_answer_.c_str(), "FL%fFR%fRL%fRR%f", &rr_state_, &fr_state_, &rl_state_, &fl_state_);

    joint_positions_[0] = (fl_state_ /180) * 3.1416;
    joint_positions_[1] = (fr_state_ /180) * 3.1416;
    joint_positions_[2] = (rl_state_ /180) * 3.1416;
    joint_positions_[3] = (rr_state_ /180) * 3.1416;
