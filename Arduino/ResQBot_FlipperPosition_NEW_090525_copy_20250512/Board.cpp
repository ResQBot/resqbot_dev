...
/**
 * Reads the HALL Bit pattern from TLE9879QXA40's CCU6 CMPSTAT register
 * @returns The 3-Bit pattern of the HALL sensor
 * @author Beatrix Dietl
 */
uint16_t TLE9879_Group::Board::readHallPattern()
{
    if (!isAvailable()) return false;

    sendMessage(READHALLPATTERN);
    uint16_t hallPattern = readAnswer();  // Read Hall pattern
    /*
    Serial.print(F("INFO: Hall Pattern Of Motor"));
    Serial.print(boardnr);
    Serial.print(F(" -> "));
    for (int i = 2; i >= 0; i--) {
      Serial.print(bitRead(hallPattern, i));
    }
    Serial.println();
    */  
    return hallPattern;
