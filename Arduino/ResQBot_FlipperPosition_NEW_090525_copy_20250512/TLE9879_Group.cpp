...
{
    //Serial.print(F("Get motor speed started:"));
    uint8_t index, end;
    if(!checkBoardNumber(boardnr, GETMOTORSPEED, index, end)) return;
    // uint16_t speed_answer=0;
    for( ; index < end; index++)
    {
...
/**
 * Reads the HALL Bit pattern from TLE9879QXA40's CCU6 CMPSTAT register from a specified board or all boards if not specifiec
 * @returns The 3-Bit pattern of the HALL sensor
 * @author Beatrix Dietl
 */
uint16_t TLE9879_Group::readHallPattern(uint8_t boardnr)
{
    uint8_t index, end;
    if (!checkBoardNumber(boardnr, READHALLPATTERN, index, end)) return;

    uint16_t hallPattern = 0;  // Store hall state
    for (; index < end; index++)  // Loop through selected boards
    {
        hallPattern = boards[index]->readHallPattern();  // Read Hall pattern
        processStatusCodes(READHALLPATTERN, index + 1);
    }

    return hallPattern;
}

void TLE9879_Group::readHallPatterns(uint16_t* buffer) {
    for (uint8_t i = 1; i <= boardCount; i++) {
        buffer[i-1] = readHallPattern(i);
    }
}
